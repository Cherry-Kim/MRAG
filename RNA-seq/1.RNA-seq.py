...
def step1_preprocessing(sample, num_threads, out_dir = 'fastqc'):
    os.system('TrimGalore-0.6.11/trim_galore --paired  --gzip -o trim_galore/ '+sample+'_1.fastq.gz '+sample+'_2.fastq.gz')
...
def step2_STAR(sample, num_threads, REF_DIR):
    os.system('STAR-2.7.11b/bin/Linux_x86_64/STAR --runThreadN '+str(num_threads)+' --genomeDir '+REF_DIR+' --readFilesIn trim_galore/'+sample+'_1_val_1.fq.gz trim_galore/'+sample+'_2_val_2.fq.gz --readFilesCommand zcat --outTmpDir '+'TEMP_'+sample+' --outFileNamePrefix '+sample+' --quantMode TranscriptomeSAM')
    os.remove(sample+'Aligned.out.sam')

def step3_RSEM(sample, REF_DIR, GENOME, num_threads):
    os.system('RSEM/rsem-calculate-expression --paired-end --bam -p '+str(num_threads)+' '+sample+'Aligned.toTranscriptome.out.bam '+REF_DIR+GENOME+' '+sample)
